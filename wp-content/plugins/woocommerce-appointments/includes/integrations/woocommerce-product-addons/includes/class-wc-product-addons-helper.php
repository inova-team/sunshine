...
class WC_Product_Addons_Helper {
	...
	public static function get_product_addons( $post_id, $prefix = false, $inc_parent = true, $inc_global = true ) {
		...
		$cache_group = 'wc-appointments-addons';
		$cache_key   = WC_Cache_Helper::get_cache_prefix( $cache_group ) . 'addons_' . $post_id . $prefix . $inc_parent . $inc_global;
		...
		return apply_filters( 'get_product_addons', $data );
	}
	 ...
	 * @return bool
	 */
	public static function is_addon_required( $addon = [] ) {
		if ( empty( $addon ) ) {
			return false;
		}
	 ...
	 */
	public static function should_display_description( $addon = [] ) {
